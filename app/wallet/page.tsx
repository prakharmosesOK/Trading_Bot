    ...
    useEffect(() => {
        ...
        const fetchWallet = async () => {
            ...
            try {
                ...
                const data = await response.json();
                setWallet({
                    total: Math.round(data.cash * 100) / 100,
                    profit: Math.round((data.equity - data.last_equity) * 100) / 100,
                    deposited: 0,
                    withdrawn: 0,
                });
            } catch (error) {
                            ...
                            <div className="mx-auto max-w-2xl lg:max-w-none">
                                ...
                                <dl className="mt-16 grid grid-cols-1 gap-0.5 overflow-hidden rounded-2xl text-center sm:grid-cols-2 lg:grid-cols-4">
                                    <div className="flex flex-col bg-white/5 p-8">
                                        ...
                                        <dd className="order-first text-3xl font-semibold tracking-tight text-white">$ {wallet.total}</dd>
                                    </div>
                                    <div className="flex flex-col bg-white/5 p-8">
                                        <dt className="text-sm font-semibold leading-6 text-gray-300">Profit</dt>
                                        <dd className="order-first text-3xl font-semibold tracking-tight text-white">$ {wallet.profit}</dd>
                                    </div>
                                    <div className="flex flex-col bg-white/5 p-8">
                                        <dt className="text-sm font-semibold leading-6 text-gray-300">Deposited</dt>
                                        <dd className="order-first text-3xl font-semibold tracking-tight text-white">$ {wallet.deposited}</dd>
                                    </div>
                                    <div className="flex flex-col bg-white/5 p-8">
                                        <dt className="text-sm font-semibold leading-6 text-gray-300">Withdrawn</dt>
                                        <dd className="order-first text-3xl font-semibold tracking-tight text-white">$ {wallet.withdrawn}</dd>
                                    </div>
                                </dl>
                            </div>
                        </div>
